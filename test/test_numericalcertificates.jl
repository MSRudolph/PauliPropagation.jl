@testset "numerical certificate Tests" begin
    ...
    wrapped_pstr = wrapcoefficients(PauliString(nq, :Z, rand(1:nq)), PauliFreqTracker)
    ...
    # PauliFreqTracker Coefficient
    wrapped_pstr = wrapcoefficients(PauliString(nq, :X, rand(1:nq)), PauliFreqTracker)
    circ = PauliPropagation._tomaskedpaulirotation(circ, pstr.nqubits)
    @test typeof(montecarlopropagation(circ, wrapped_pstr)) <: Tuple{typeof(wrapped_pstr),Bool}
    wrapped_pstr = wrapcoefficients(PauliString(nq, :Y, rand(1:nq)), PauliFreqTracker)
    circ = PauliPropagation._tomaskedpaulirotation(circ, pstr.nqubits)
